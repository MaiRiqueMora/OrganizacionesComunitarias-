 ...
 * 
 * DESCRIPCIÓN:
 * API REST para gestión de documentos de organizaciones.
 * Maneja el almacenamiento, descarga y metadata de archivos adjuntos.
 * 
 * FUNCIONALIDADES:
 * - Subida de archivos a organizaciones
 * - Descarga segura de documentos
 * - Listado de documentos por organización
 * - Eliminación de documentos físicos y registros
 * - Gestión de metadata (tipo, descripción, etc.)
 * - Control de acceso y permisos
 * 
 * ENDPOINTS:
 * - GET    /api/documentos.php                    - Listar documentos
 * - GET    /api/documentos.php?download=X         - Descargar documento
 * - POST   /api/documentos.php                    - Subir nuevo documento
 * - DELETE /api/documentos.php?id=X              - Eliminar documento
 * 
 * PARÁMETROS:
 * - org_id: ID de organización (filtro)
 * - download: ID de documento para descarga
 * - id: ID de documento para eliminación
 * 
 ...
 * 
 * ALMACENAMIENTO:
 * - Directorio: uploads/organizaciones/[org_id]/
 * - Nombres únicos con timestamp
 * - Metadata en base de datos MySQL/MariaDB 
 * - Eliminación física al borrar registro
 * 
 * RESPUESTAS:
 * - JSON para operaciones CRUD
 * - Descarga directa para archivos
 * - Errores detallados con código HTTP
 * 
...
if ($method === 'GET' && !empty($_GET['download'])) {
    ...
    $stmt = $pdo->prepare("SELECT id,organizacion_id,nombre,tipo,archivo_path,mime_type,tamanio,descripcion,uploaded_by,created_at FROM documentos WHERE id=?");
