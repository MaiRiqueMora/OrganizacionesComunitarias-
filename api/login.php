...
/**
 * Registrar acceso exitoso en la tabla user_access_logs
 */
function logAcceso(int $userId, string $username, array $context): void {
    try {
        $db = getDB();
        $stmt = $db->prepare('INSERT INTO user_access_logs (user_id, ip_address, user_agent) VALUES (:user_id, :ip_address, :user_agent)');
        $stmt->execute([
            ':user_id' => $userId,
        ...
        Logger::getInstance()->logErrorToDatabase('Error al registrar acceso: ' . $e->getMessage(), $userId, json_encode($context));
    }
}
