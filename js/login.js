document.addEventListener('DOMContentLoaded', async () => {
  if (await sessionExists()) { window.location.href = AUTH.dashboardPage; return; }
  ...
  window.doLogin = async function(){
    ...
    if(r.ok){ window.location.href=AUTH.dashboardPage; }
    else{ showAlert('loginError',r.error||'Error al iniciar sesión.'); btn.disabled=false; btnTxt.style.display=''; spin.style.display='none'; pInp.value=''; pInp.focus(); }
  };
});
