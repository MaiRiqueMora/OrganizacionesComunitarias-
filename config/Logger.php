...
class Logger {
     ...
     */
    public function logErrorToDatabase(string $message, ?int $userId = null, ?string $context = null): void {
        try {
            $db = new PDO('sqlite:' . __DIR__ . '/../database/munidb.sqlite');
            $stmt = $db->prepare('INSERT INTO error_logs (error_message, error_context, user_id) VALUES (:message, :context, :user_id)');
            $stmt->execute([
                ':message' => $message,
                ':context' => $context,
                ':user_id' => $userId
            ...
            // Si ocurre un error al registrar, escribir en el archivo de log
            $this->log(self::ERROR, 'Error al registrar en la base de datos: ' . $e->getMessage());
        }
    }
}
