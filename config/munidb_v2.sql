...
-- Tabla para registrar errores detallados
...
-- Tabla para registrar el historial de accesos de los usuarios
...
try {
    ...
    Logger::getInstance()->logErrorToDatabase($e->getMessage(), $_SESSION['user_id'] ?? null, json_encode($_REQUEST));
...
try {
    ...
    Logger::getInstance()->logErrorToDatabase($e->getMessage(), $userId, 'Creación de usuario fallida');
    throw $e;
}